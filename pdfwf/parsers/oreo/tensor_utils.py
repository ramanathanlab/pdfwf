...
from utils.general import non_max_suppression, scale_boxes, make_divisible
...
def infer_file_type(doc_dir : Union[Path, str]) -> str:
    ...
    # check dir existance
    assert os.path.isdir(doc_dir), f"Directory `doc_dir`={doc_dir} does not exist."
    assert len(os.listdir(doc_dir))>0, f"Directory `doc_dir` exists but is empty."
    
    # most frequent file format
    suffix_counts = Counter([Path(f).suffix for f in os.listdir(doc_dir) if '.' in f])
    ...
    if(mode_suffix in FileType.__members__):
        ...
        raise ValueError(f"The mode file type in the directory '{mode_suffix}' does not match any of the allowed file types: '.pdf', '.doc', '.docx'")
        
    return file_type

class DocDataset(Dataset):
    def __init__(self, doc_dir: Union[Path, str], meta_only:bool=False, target_heigth:int=1280, file_type:str='pdf') -> None:
        
        assert file_type in ['pdf', 'doc', 'docx', 'auto'], "DocDataset can handle PDFs (file_type=`pdf`) or Word documents (file_type=`doc(x)`)"
        if(file_type=='auto'):
            file_type = infer_file_type(doc_dir)
            
        if(file_type!='pdf'):
            raise NotImplementedError("Only file_type that is supported currently is `pdf`")
        
        self.doc_file_paths = sorted([Path(doc_dir) / f for f in os.listdir(doc_dir) if f.lower().endswith(f'.{file_type}')])
    ...
    def __getitem__(self, idx: int) -> torch.Tensor:
        ...
        # - entire paper
            ...
            rel_page_idx = idx - (0 if doc_index==0 else int(self.doc_csum[doc_index-1]))
        ...
        output = docpage_to_tensor(page, self.target_heigth, fill_value=1)  # (C, H, W)
...
def pre_processing(results:torch.Tensor,
                   file_ids:List[int],
                   ...
                   collapse_agnosticly:bool=True) -> torch.Tensor:
    """
    ...
    y = non_max_suppression(results, iou_thres=iou_thres, conf_thres=conf_thres, agnostic=collapse_agnosticly)  
    
    # post-process bbox predictions
    y = get_y_indexed(y, file_ids=file_ids, x_delta=x_delta, freq_thresh=freq_thresh, rel_class_ids=rel_class_ids)
    
    return y

def get_y_indexed(y:torch.Tensor, 
                  file_ids:List[int],
                  rel_class_ids:List[int], 
                  x_delta:int=200, 
    ...
    y_indexed = [torch.cat((page_tensor, torch.full((page_tensor.size(0), 1), i, device=page_tensor.device)), dim=1) for i, page_tensor in enumerate(y)]
    
    y_batch = torch.cat(y_indexed, dim=0)

    # subset to localizing condition column classes
    current_device = y_batch.device
    loc_cond_col = torch.zeros(y_batch.size(0), dtype=torch.bool).to(current_device)

    # define the condition (class-conditioned, x-axis modes)
    for cls_label in rel_class_ids:
        loc_cond_col = loc_cond_col | torch.isclose(y_batch[:,cls_column], torch.tensor(1. * cls_label))
    ...
    x_Mid_rounded = torch.clamp(x_Mid_rounded, min=x_delta, max=1200).reshape(x_Mid_rounded.size()[0],1)
    ...
    y_Mid_rounded = torch.clamp(y_Min_rounded, min=y_delta, max=1300).reshape(y_Min_rounded.size()[0],1)
    
    # - augment w/ midpoints
    tensor_aug = torch.cat((y_batch, x_Mid_rounded.reshape(x_Mid_rounded.size()[0], 1)), dim=1)

    # - compute midpoints (along y-axis)
    y_Mid = 0.5 * (y_batch[:,ymin_column] + y_batch[:,ymax_column])
    tensor_aug = torch.cat((tensor_aug, y_Mid.reshape(x_Mid.size()[0], 1)), dim=1)

    # empty mode column: col_index, row_idx, width, height, patch_order_idx, file_idx, element idx
    zeros_columns = torch.zeros(tensor_aug.size(0), 7, device=tensor_aug.device)
    tensor_aug = torch.cat((tensor_aug, zeros_columns), dim=1)

    # for each page (if localizing class): mode estimation/assignment
    for page_idx in torch.unique(y_batch[loc_cond_col, page_idx_column], return_counts=False):
        # condition column (txt element & page idx)
        idx_cond_col = torch.isclose(y_batch[:,page_idx_column], page_idx)

        # x-axis mode estimation
        modes, counts = torch.unique(x_Mid_rounded[idx_cond_col & loc_cond_col], return_counts=True)
        freqs = counts.float() / len(x_Mid_rounded[idx_cond_col]) if len(x_Mid_rounded[idx_cond_col]) > 0 else 1
        robust_modes = modes[freqs > freq_thresh]
        robust_modes = robust_modes if robust_modes.size()[0]>0 else torch.tensor(0)

        # column_index
        # = = = = = = =
        # - compute mode distances
        midpoint_mode_distances = torch.abs(tensor_aug[:, midpoint_x_column].unsqueeze(1) - robust_modes)
        # - closest mode index
        min_indices = torch.argmin(midpoint_mode_distances, dim=1)
        # - column_index
        tensor_aug[idx_cond_col, col_idx_column] = min_indices[idx_cond_col].float()

        # row_index
        # = = = = = 
        # - round y_min values
        y_bins = torch.arange(0, 1280, y_bin_width).to(tensor_aug.device)
        y_min_rounded = torch.bucketize(tensor_aug[idx_cond_col, ymin_column], y_bins).to(torch.float)
        
        # y-axis mode estimation
        modes, counts = torch.unique(y_Mid_rounded[idx_cond_col], return_counts=True)
        freqs = counts.float() / len(x_Mid_rounded[idx_cond_col]) if len(y_Mid_rounded[idx_cond_col]) > 0 else 1
        y_robust_modes = modes[freqs > y_freq_thresh]
        y_robust_modes = y_robust_modes if y_robust_modes.size()[0]>0 else torch.tensor(0)

        # column_index
        # - compute mode distances
        y_min_mode_distances = torch.abs(tensor_aug[:, ymin_column].unsqueeze(1) - y_robust_modes)
        # - closest mode index
        y_min_indices = torch.argmin(y_min_mode_distances, dim=1)
        # - column_index
        tensor_aug[idx_cond_col, row_idx_column] = y_min_indices[idx_cond_col].float()
        
        # lexsort : infer order of patches
        sorted_indices = torch_lexsort(tensor_aug[idx_cond_col,:], keys=[ymin_column, row_idx_column, col_idx_column])
        ranks = torch.empty_like(sorted_indices, dtype=torch.float)
        ranks[sorted_indices] = 1. * torch.arange(len(sorted_indices)).to(tensor_aug.device)
        tensor_aug[idx_cond_col, order_idx_column] = 1. * ranks
    ...
    tensor_aug[:,file_idx_column] = torch.Tensor(file_ids).to(tensor_aug.device)[tensor_aug.to(torch.int)[:,page_idx_column]]
    
    # assign elementwise idx (last column)
    tensor_aug[:,idx_column] = torch.arange(tensor_aug.size()[0]).to(torch.float).to(tensor_aug.device)
    
    return tensor_aug


def subset_y_by_class(y_batch:torch.Tensor, rel_class_ids:List[int]) -> torch.Tensor:
    '''
    ...
    '''
    assert min(rel_class_ids)>=0 and max(rel_class_ids)<=21, "Class labels should be "
    ...
    # condtion
    condition_tensor = torch.zeros(data_tensor.size(0), dtype=torch.bool).to(current_device)

    # Define the condition
    for cls_label in rel_class_ids:
        condition_tensor = condition_tensor | torch.isclose(cls_column, torch.tensor(1. * cls_label))
...
def docpage_to_tensor(page:fitz.Page, target_height:int=1280, target_width:int=960, fill_value:int=0) -> torch.Tensor:
    """
    ...
    """
    assert 640<=target_height<=1280, "Target height should be in [640, 1280]"
    
    # re-scale (maintain multiple of 64 along width and height)
    scale_factor_y = target_height / page.mediabox_size.y
    scale_factor_x = ((scale_factor_y * page.mediabox_size.x // 64) * 64) / page.mediabox_size.x
    ...
    if(x.size()!=torch.Size([3, target_height, target_width])):
        x = pad_zeros(x, target_height=target_height, target_width=target_width, fill_value=fill_value)
        x = center_crop(x, target_height=target_height, target_width=target_width)
    
    return x


class ResizeAndCenterCropTensor(object):
    ...
    def __call__(self, img_tensor):
        ...
        img_tensor = img_tensor[:, start_h:start_h + self.reference_height, start_w:start_w + self.reference_width]

        return img_tensor
    
    
def pad_zeros(x:torch.Tensor, target_height:int=1280, target_width:int=960, fill_value:int=0) -> torch.Tensor:
    """
    ...
    if target_height > x.size(1):
        zero_cols = fill_value * torch.ones((x.size(0), target_height-x.size(1), x.size(2)), device=x.device)
        x = torch.cat((x, zero_cols), dim=1)
    # width
    if target_width > x.size(2):
        zero_rows = fill_value * torch.ones((x.size(0), x.size(1), target_width-x.size(2)), device=x.device)
        x = torch.cat((x, zero_rows), dim=2)
    
    return x

def center_crop(x:torch.Tensor, target_height:int=1280, target_width:int=960) -> torch.Tensor:
    """
    ...
    # subset
    x = x[:, h_delta:(target_height+h_delta), w_delta:(target_width+w_delta)]
...
def pad_patch(patch:torch.Tensor, patch_value:int=255) -> torch.Tensor:
    ...
    padded_patch = F.pad(patch, (pad_left, pad_right, pad_top, pad_bottom), mode='constant', value=patch_value)
    
    return padded_patch

def resize_patch(patch:torch.Tensor, max_length:int=420, mode:str='bilinear') -> torch.Tensor:
    """
    ...
    height, width = patch.size()[1:]
     ...
     # rescale
    width_scale  = 1. * max_length / width
    height_scale = 1. * max_length / height
    ...
    t_interpol = F.interpolate(patch.unsqueeze(0), size=(new_height, new_width), mode=mode, align_corners=False, antialias=True)[0,:,:] #.to(torch.uint8)
    
    return t_interpol.round().to(torch.int)
    
def normalize_pad(t:torch.Tensor, mean:List[float] = [0.485, 0.456, 0.406], std:List[float] = [0.229, 0.224, 0.225]):
    """
...
def tens_proc(t:torch.Tensor, max_length:int=420, mode:str='bilinear', patch_value:float=1.0):
    """
...
def get_patch_batch(tensors:torch.Tensor, y:torch.Tensor, max_width:int=420, max_height:int=420, dtype=torch.float16) -> torch.Tensor:
    """
    ...
    # placeholder batch
    patch_batch = torch.zeros((N, 3, max_height, max_width), device=tensors.device, dtype=tensors.dtype)
    ...
    for i in range(N):
        ...
        # process patch
        patch = tens_proc(t=patch*255., max_length=420, mode='bilinear', patch_value=1.0)
...
def vectorized_patch_batch(tensors:torch.Tensor, 
                           y:torch.Tensor, 
                           max_width:int=420, 
                           max_height:int=420, 
    ...
    # transform
    tensors = tensors.to(torch.float) * 255.
    ...
    # rescale & normalize
    patch_batch = patch_batch / 255.
    patch_batch = normalize_pad(patch_batch)
    
    return patch_batch

def get_patch_list(tensors:torch.Tensor, 
                   y:torch.Tensor, 
                   max_width:int=420, 
                   max_height:int=420, 
                   mean:List[float] = [0.485, 0.456, 0.406], 
                   std:List[float] = [0.229, 0.224, 0.225],
                   offset:int=0) -> Tuple[torch.Tensor, List[int]]:
    """
    Given a batch of images (BxCxHxW) and a tensor of meta information on patches, return the batch of patch tensors.
    ...
    for i in range(N):
        ...
        if(offset > 0):
            ...
            x_max, y_max = min(page_w, x_max + offset), min(page_h, y_max + offset)
        # extract patch
        patch = tensors[b,:, y_min:y_max, x_min:x_max]    # challenge
        
        # append list
        patch_list.append(patch / 255.)
    
    return patch_list


def patch_list_to_tensor(grouped_patch_list:List[torch.Tensor],
                         dtype:torch.dtype,
                         max_width:int=420, 
                         max_height:int=420, 
                         mean:List[float] = [0.485, 0.456, 0.406], 
    ...
    # transform
    packed_batch = torch.zeros((N_eff, 3, max_height, max_width)).to(grouped_patch_list[0].device) * 255.
    ...
    for i,patch in enumerate(grouped_patch_list):
        ...
        # resize
        t = resize_patch(patch, max_length=max_width, mode='bilinear')   # challenge 

        # assign
        packed_batch[i,:,:,:] = pad_patch(t, 255.0)
        
    # rescale & normalize
    packed_batch = packed_batch / 255.
...
def lower_bound_patch_batch(tensors:torch.Tensor, y:torch.Tensor, max_width:int=420, max_height:int=420, mean:List[float] = [0.485, 0.456, 0.406], std:List[float] = [0.229, 0.224, 0.225]) -> torch.Tensor:
    """
...
def merge_patches_into_row(row_patch_list:List[torch.Tensor], 
                           btm_pad:int, 
                           sep_flag:bool=False, 
    ...
    row_tensor = torch.cat([F.pad(patch, (0, 0, row_height-patch.size()[1], 0), value=1.0) for patch in row_patch_list], dim=2) # top-padding
...
def merge_rows_into_patch(list_of_row_tensors:List[torch.Tensor]) -> torch.Tensor:
    """
    ...
    patch_width = max([row_tensor.size()[2] for row_tensor in list_of_row_tensors])
        
    patch_tensor = torch.cat([F.pad(row_tensor, (0, patch_width-row_tensor.size()[2]), value=1.0) for row_tensor in list_of_row_tensors], dim=1)
    
    return patch_tensor

def grouped_patch_list(patch_list:List, 
                       y:torch.Tensor, 
                       unpackable_class_ids:List[int],
                       by:List[str]=['file_id', 'cls']) -> List[List[torch.Tensor]]:
    """
    ...
    # check input
    assert len(y.size())==2 and y.size()[1]==16, "Tensor of patch meta data must be 2-dimensional with 16 columns."
    assert len(patch_list)==len(y), "List of raw patches and length of metadata tensor must coincide."
    assert set(by).issubset({'file_id', 'cls', 'page_id'}), "Can only group by `file_id` (within pdf file), `cls` (same class label id), or `page_id` (within same pdf page)."
    ...
    unpack_mask    = torch.any(y[:, cls_column][:, None] == torch.Tensor(list(unpackable_class_ids)).to(y.device), dim=1)
    ...
    y_aug[unpack_indices, unpack_col] = torch.arange(len(unpack_indices), device=y_aug.device, dtype=y_aug.dtype)+1
    ...
    y_aug[:, unpack_col] = torch.cumsum(y_aug[:, unpack_col], dim=0) + (1000 * y_aug[:, unpack_col])

    # unique entries
    _, index_groups = torch.unique(y_aug[:,keys_aug], return_inverse=True, dim=0)
    unique_vals     = torch.unique(index_groups).detach().tolist()

    # subset index list
    subset_indices = [index_set[(index_groups==v)].tolist() for v in unique_vals]
    subset_indices = sorted(subset_indices, key=lambda x: x[0])

    # subset list
    patch_sublists = [[patch_list[i] for i in sub_ind] for sub_ind in subset_indices]

    return patch_sublists, subset_indices

def grouped_patch_list_LEGACY(patch_list:List, y:torch.Tensor, by=['file_id', 'cls']) -> List[List[torch.Tensor]]:
    """
    Given a list of variably-sized patches, groups them by paper and cls type. Each group represented by a sublist in the returned list.
    def get_packed_patch_list
    Args:
...
def get_packed_patch_list(patch_list:List[torch.Tensor], 
                          ...
                          max_width:int=420, 
                          max_height:int=420, 
                          alpha:float=0.5) -> List[torch.Tensor]:
    """
    ...
    for j,patch in enumerate(patch_list):
        ...
        if(current_row_width + w < max_width):
            ...
            if(len(current_row) > 0):
                list_of_rows.append(merge_patches_into_row(current_row, btm_pad=btm_pad, sep_flag=sep_flag, alpha=alpha))
    ...
    if(current_row):
        ...
        if(list_of_rows):
            ...
            list_of_rows = [merge_patches_into_row(current_row, btm_pad=btm_pad, sep_flag=sep_flag, alpha=alpha)]
            list_of_rows_indices = current_row_indices
...
def get_separator_tensor(img_path:Path=Path('/home/siebenschuh/Projects/N-O-REO/sep_image/square.png')) -> torch.Tensor:
    """
    Loads the separator symbol from an image and turns it into a tensor
    """
    
    img_path = Path(img_path)
    assert os.path.isfile(img_path), f"Path to separator symbol image {img_path} does not exist:"
    assert img_path.suffix=='.png', "Image path exists but expected it to be a .png"
...
def restate_global_patch_indices(packed_indices:List[List[List[int]]]) -> List[List[List[int]]]:
    """
    ...
    for j, p_patch in enumerate(packed_indices):
        if(j > 0):
            j_acc += (max([1] if torch.is_tensor(packed_indices[j-1]) else max(packed_indices[j-1])) + 1)
        new_packed_indices.append([[j+j_acc for j in p_row] for p_row in p_patch])

    assert len(packed_indices)==len(new_packed_indices), "Length should coicide"
    assert len(packed_indices[0])==len(new_packed_indices[0]), "Inner list lengths do not coincide."
    
    return new_packed_indices


def load_spv05_categories(spv05_category_file_path:Path = Path("./meta/spv05_categories.yaml")):
    """
    ...
    """
    # read yaml
    with open(spv05_category_file_path, "r") as file:
        spv05_meta_file = yaml.safe_load(file)

    # categories
    assert all([k in spv05_meta_file.keys() for k in ['names', 'groups']]), "File exists but doesn't have key `names` and `groups` in it."
    inv_categories  = spv05_meta_file['names']
    
    groups = spv05_meta_file['groups']
    categories = {v:k for k,v in inv_categories.items()}
    
    assert list(categories.values())==list(range(21)), "SPv05 has 21 classes that are assumed to be represented."
    
    return categories, groups


def get_relevant_text_classes(file_type:str='pdf',
...
def get_relevant_visual_classes(file_type:str='pdf',
                                table_flag:bool=False, 
                                fig_flag:bool=False,
                                spv05_category_file_path:Path = Path("./meta/spv05_categories.yaml")):
    """
...
def get_packed_patch_tensor(tensors:torch.Tensor, 
                            y:torch.Tensor, 
                            rel_class_ids:List[int], 
                            unpackable_class_ids:List[int]=None, # sharp corner : lists as arguments in Python "is a bitch"
    ...
    assert len(tensors.size())==4, "`tensors` must be 4-dimensional (BxCxHxW)."
    ...
    # constants
    file_idx_column, page_idx_column, order_idx_column, cls_column = 14, 6, 13, 5
    ...
    patch_ids_in_order = torch_lexsort(y_subset[:,[file_idx_column, page_idx_column, order_idx_column, cls_column]], keys=[2,1,0])
    ...
    if(sorted(rel_class_ids)!=sorted(unpackable_class_ids)):
        ...
        # group patches
        grouped_patches, group_indices = grouped_patch_list(patch_list, y_subset, by=by, unpackable_class_ids=unpackable_class_ids)  
        
        # packing patches & log indices
        packed_patches_and_indices = [get_packed_patch_list(groups, sep_tensor=sep_symbol_tensor, btm_pad=btm_pad, return_indices=True, 
                                                            sep_flag=sep_flag, sep_symbol_flag=sep_symbol_flag) for groups in grouped_patches]
        
        # single list of patches
        packed_patches = [pair[0] for pair in packed_patches_and_indices]
        packed_indices = restate_global_patch_indices([pair[1] for pair in packed_patches_and_indices])
        ...
        flat_indices = [f for f in flat_indices if len(f) > 0] # del. empty list elements
        
        index_quadruplet = y_subset[[f[0] for f in flat_indices],:][:,[file_idx_column, page_idx_column, order_idx_column, cls_column]].to(torch.int)
        ...
        index_quadruplet = y_subset[:,[file_idx_column, page_idx_column, order_idx_column, cls_column]].to(torch.int)
...
def update_main_content_dict(doc_dict:defaultdict(list), 
    ...
    assert len(index_quadruplet)==len(text_results), "Meta data entries and list of text elements must coincide."
    ...
    if(dset_name=='SPv05'):
        ...
        meta_class_id_tensor = torch.tensor([1, 3, 6, 10, 11, 12], device=index_quadruplet.device, dtype=torch.int)
        meta_class_lab_list  = ['Title', 'Equations', 'Keywords', 'Author', 'Institution', 'Date']
        meta_added_to_text   = ['Title']
    else:
        raise NotImplementedError("Text-based classification only applicable to meta classes of SPv05. Register another dataset's meta class id's here.")

    # index
    file_index_set = torch.arange(len(index_quadruplet), device=index_quadruplet.device, dtype=index_quadruplet.dtype)

    # meta classes
    all_index_dict = {}
    for cls_meta_id, cls_meta_label in zip(meta_class_id_tensor, meta_class_lab_list):
        all_index_dict[cls_meta_label] = torch.isclose(index_quadruplet[:,cls_column], cls_meta_id)
    # text (remaining categories)
    tensors_excl_meta       = torch.stack([all_index_dict[lab] for lab in list(set(meta_class_lab_list).difference(meta_added_to_text))])
    all_index_dict['Text'] = torch.ones_like(index_quadruplet[:,cls_column]).bool() & (~torch.any(tensors_excl_meta, dim=0))
    ...
    for file_id, file_id_cpu in zip(curr_file_ids, curr_file_ids_on_cpu):
        ...
        file_id_indices    = torch.isclose(index_quadruplet[:,file_id_column], file_id)
        ...
        for cls_label in all_index_dict.keys():
            cls_meta_patch_indices = (file_id_indices & all_index_dict[cls_label])
            file_meta_index_subset = file_index_set[cls_meta_patch_indices].tolist()
            doc_dict[cls_label][file_id_cpu].extend([text_results[i] for i in file_meta_index_subset])
            
            # visual assets
            if(vis_path_dict):
                for vis_cls_label in vis_path_dict.keys():
                    if(file_id_cpu in vis_path_dict[vis_cls_label].keys()):
                        doc_dict[vis_cat_map[vis_cls_label]][file_id_cpu].extend(vis_path_dict[vis_cls_label][file_id_cpu])
            
    
    return doc_dict


def store_completed_docs(main_doc_dict:defaultdict(list), 
                         curr_file_ids:torch.Tensor,
    ...
    """
    # check dir existance
    assert os.path.isdir(store_dir), f"The directory to which the text output files are stored `store_dir`={store_dir} does not exist."
    ...
    if(store_all_now):
        ...
        completed_file_ids = list(set(main_doc_dict.keys()).difference(set(curr_file_ids_on_cpu)))
    
    assert max(completed_file_ids)<len(doc_file_paths), "File ids extend range of `doc_file_paths`"
    ...
    if(len(completed_file_ids) > 0):
        for compl_file_id in completed_file_ids:
            ...
            file_store_path = store_dir / f"{doc_file_paths[compl_file_id].stem}.txt"
            with open(file_store_path, 'w') as file:
                file.write(processed_file_text)
    pass


def assign_text_inferred_meta_classes(txt_cls_model, 
                                      tokenizer, 
                                      batch_size:int, 
                                      index_quadruplet:torch.Tensor, 
                                      text_results:List[str], 
                                      dset_name:str='SPv05') -> torch.Tensor:
    ''' 
    ...
    # dataset
    if(dset_name=='SPv05'):
        class_id_mapping_tensor = torch.tensor([1, 6, 10, 11, 12], device=index_quadruplet.device, dtype=torch.int)
    ...
    meta_indices      = torch.arange(len(index_quadruplet), device=index_quadruplet.device)[mask]
    ...
    for j in range(n_eff+1):
        ...
        # run inference
        inputs = tokenizer(meta_text_results[j_min : j_max], 
                           return_tensors="pt", padding=True).to(index_quadruplet.device)
...
def extract_file_specific_doc_dict(doc_dict, file_id:int, LaTex2Text:LatexNodes2Text):
    """
...
def store_completed_docs(doc_dict:defaultdict(lambda: defaultdict(list)), 
                         curr_file_ids:torch.Tensor,
                         doc_file_paths:List[Path],
                         store_dir:Path,
    ...
    if(store_all_now):
        ...
        curr_file_ids_on_cpu = curr_file_ids.tolist()
        completed_file_ids = list(set(doc_dict['Text'].keys()).difference(set(curr_file_ids_on_cpu)))
    ...
    if(len(completed_file_ids) > 0):
        for compl_file_id in completed_file_ids:
            # extract
            one_doc_dict = extract_file_specific_doc_dict(doc_dict, compl_file_id, LaTex2Text)

            # save TODO. Uses "dataset" below
            file_store_path = store_dir / f"{doc_file_paths[compl_file_id].stem}.json"
            with open(file_store_path, 'w') as f:
                json.dump(one_doc_dict, f)
    pass

def store_visuals(tensors:torch.Tensor, 
                  y:torch.Tensor, 
    ...
    assert os.path.isdir(output_dir), "Target directory `output_dir` must exist."
    ...
    y_sub_visual = subset_y_by_class(y_batch=y, rel_class_ids=visual_class_ids).round().to(torch.int)
    
    # clip
    _, C, H, W = tensors.size()
    y_sub_visual[:, ymin_column] = y_sub_visual[:,ymin_column].clamp(min=0, max=H)
    y_sub_visual[:, ymax_column] = y_sub_visual[:,ymax_column].clamp(min=0, max=H)
    y_sub_visual[:, xmin_column] = y_sub_visual[:,xmin_column].clamp(min=0, max=W)
    y_sub_visual[:, xmax_column] = y_sub_visual[:,xmax_column].clamp(min=0, max=W)

    # mapping file_id -> filename
    file_id_name_mapping = {f_id:f_name for (f_id, f_name) in zip(file_ids, file_paths)}
    ...
    for i, (cls_id, file_id) in enumerate(zip(cls_id_list, file_id_list)):
        ...
        # extract patch
        img_patch = transforms.ToPILImage()(tensors[b,:, y_min:y_max, x_min:x_max])
        
        # derive file path (incl. sub-directory)
        patch_file_path = output_dir / inv_rel_visual_classes[cls_id] / f"{inv_rel_visual_classes[cls_id]}_{file_id_name_mapping[file_id].stem}_{i_dict[cls_id]}.png"
        i_dict[cls_id] += 1 
