...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='cuda',
        help='Device on which majority of data processing occurs.',
    )
    ...
    parser.add_argument(
        '--max_page_to_store_visuals',
        type=int,
        default=1000,
        help='Max. number of pages in the dset for which table/figure saving is allowed.',
    )
    ...
    assert os.path.isdir(
        input_dir
    ), f'Input directory is invalid; does not exist: `input_dir`={input_dir}'
    assert os.path.isdir(
        output_dir
    ), f'Output directory is invalid; does not exist: `output_dir`={output_dir}'
    assert device in [
        'cuda',
        'cpu',
    ], 'Chosen `device` must be either `cuda` or `cpu`.'

    # device availability
    if device == 'cuda' and not (torch.cuda.is_available()):
        print(
            '--device `cuda` was chosen but is not available. CPU-processing only.'
        )
    ...
    ), f'Path to weights of detection model does not exist: {detection_weights_path}'
    assert os.path.isdir(
        txt_cls_weights_path
    ), f'Path to weights of text classification model does not exist {txt_cls_weights_path}'
    ...
    if detect_only and (
        equation_flag or table_flag or fig_flag or secondary_meta or meta_only
    ):
        parser.error(
    ...
    rel_txt_classes = get_relevant_text_classes(
        'pdf', meta_only, equation_flag, table_flag, fig_flag, secondary_meta
    )
    ...
    # load dataset
    dataset = DocDataset(
        doc_dir=input_dir, meta_only=meta_only, file_type=file_type
    )
    ...
    # classes
    meta_cls_tensor = torch.tensor(
        list(rel_meta_txt_classes.values()), device=device, dtype=torch.int
    )
    ...
    with torch.no_grad():
        for i, batch in enumerate(data_loader):
            ...
            y = pre_processing(
                results, file_ids, rel_txt_classes.values(), iou_thres=0.001
            )
